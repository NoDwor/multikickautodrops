...
async def view_stream(username, category_id, cookies_file="cookies.txt", progress_file="current_views.json"):
    cookies = cookies_manager.load_cookies(cookies_file)
    token = kick.get_token_with_cookies(cookies)
    streamerid = kick.get_channel_id(username)
    status = await kick.connection_channel(streamerid, username, category_id, token, progress_file)
    return status
...
async def check_campaigns_claim_status(cookies_file="cookies.txt", progress_file="current_views.json"):
    cookies = cookies_manager.load_cookies(cookies_file)
    kickdata = kick.get_drops_progress(cookies)
    formatter.sync_drops_data(kickdata, cookies, filepath=progress_file)
